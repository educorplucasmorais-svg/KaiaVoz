...
remindersRouter.post('/', async (req, res) => {
  try {
    ...
    if (when) {
      ...
      if (!whenAt) whenAt = parsePtBrNatural(when, ref)
      if (!whenAt) {
    ...
    if (prisma) {
      const reminder = await prisma.reminder.create({ data: { title, whenAt, notes } })
      return res.json({ success: true, data: reminder })
    } else {
      const item: MemReminder = { id: memory.seq++, title, notes: notes ?? null, whenAt, created: new Date() }
...
remindersRouter.get('/', async (_req, res) => {
  try {
    if (prisma) {
      ...
      const reminders = memory.reminders.slice().sort((a, b) => a.whenAt.getTime() - b.whenAt.getTime())
      return res.json({ success: true, data: reminders })
    }
  } catch (err: any) {
    return res.status(500).json({ success: false, error: err?.message || 'Unknown error' })
